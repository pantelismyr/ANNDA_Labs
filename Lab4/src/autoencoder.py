...
def getAutoencoder(img_dim, encoding_dim):
    # input (784d) placeholder
    input_img = Input(shape=(img_dim, ))
    # hidden layer (encoded)
    ...
    decoded = Dense(img_dim, activation='sigmoid', use_bias=True, kernel_initializer=weight_init, 
                    bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, 
...
def train_test(x_train, x_test, loss_f, opt, encoding_dim, epochs, batch_size, earlyStop):
    # get the models
    autoencoder, encoder, decoder = getAutoencoder(len(x_train[0]), encoding_dim)
...
def main():
    ...
    if mode == '1':
        ...
        decoded_imgs, final_weights, optimal_val_loss = train_test(np.asarray(x_train), np.asarray(x_test), loss_f, opt=adam, encoding_dim=100, epochs=200, batch_size=250, earlyStop='On')
    elif mode == '2':
        #--- search for the optimal number of nodes
        unit_ls = [50, 75, 100, 150]
        val_losses_ls = []
        for units in unit_ls:
            decoded_imgs, final_weights, optimal_val_loss = train_test(x_train, x_test, loss_f, opt=adam, encoding_dim=units, epochs=10, batch_size=250, earlyStop='On')
